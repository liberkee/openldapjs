...
describe('Testing the async LDAP paged search ', () => {
  ...
  it('should reject if scope parameter doesn\'t exist', () => {
    return adminLDAP.pagedSearch(searchBase, 'noScope', config.ldapSearch.filterObjSpecific,
      pageSize)
      .then(() => {
        should.fail('Didn\'t expect success');
      })
  ...
  it('should return search results with pagesize 10',
    function pageSearchTime(next) {
      ...
      adminLDAP.pagedSearch(searchBase, searchScope.subtree, config.ldapSearch.filterObjAll,
        pageSize)
        .then((res) => {
          res.on('data', (data) => {
            numberPage += 1;
