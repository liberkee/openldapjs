...
std::vector<LDAPControl *> LdapControls::CreateModificationControls(const v8::Local<v8::Array> &control_handle) {
  ...
  for(auto& ctrl : ctrls) {
    ...
    const auto value_controles = v8::Local<v8::Array>::Cast(controls->Get(Nan::New(constants::changeValueMember).ToLocalChecked()));
    ...
    v8::String::Utf8Value controlOperation(controls->Get(Nan::New(constants::changeOidMember).ToLocalChecked()));

    if(std::strcmp(*controlOperation, constants::postread) == 0) {
      ctrl->ldctl_oid = (char *)LDAP_CONTROL_POST_READ;
    } else if (std::strcmp(*controlOperation, constants::preread) == 0) {
      ctrl->ldctl_oid = (char *)LDAP_CONTROL_PRE_READ;
    } else {
      return std::vector<LDAPControl *>();
    }

    auto isCritical = v8::Local<v8::Number>::Cast(controls->Get(Nan::New(constants::changeIsCriticalMember).ToLocalChecked()));
...
std::string LdapControls::PrintModificationControls (LDAP *ld, LDAPMessage *resultMsg) {
  ...
  ldap_parse_result(ld, resultMsg, nullptr, nullptr, nullptr, nullptr, &serverCTL, 0);
  auto i = 0;
