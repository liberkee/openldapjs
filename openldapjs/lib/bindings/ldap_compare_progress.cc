...
LDAPCompareProgress::LDAPCompareProgress(Callback *callback, Callback *progress, LDAP *ld,
    int msgID)
: AsyncProgressWorker(callback), progress(progress), ld(ld),
msgID(msgID) {}

void LDAPCompareProgress::Execute(const AsyncProgressWorker::ExecutionProgress &progress) {
  ...
  void LDAPCompareProgress::HandleProgressCallback(const char *data, size_t size) {
