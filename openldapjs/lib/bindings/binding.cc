...
class LDAPPagedSearchProgress : public AsyncProgressWorker
{
  ...
  public:
    LDAPPagedSearchProgress(Callback *callback, Callback *progress, LDAP *ld, std::string base, int scope,
                            std::string filter, const std::string& cookieID,
                            int pgSize, const std::shared_ptr<std::map<std::string, berval*>>& cookies)
      : AsyncProgressWorker(callback), progress(progress), ld(ld), base(base), scope(scope),
        filter(filter),cookieID(cookieID), pageSize(pgSize), cookies(cookies)
    {
  ...
  private:
    ...
    static NAN_METHOD(pagedSearch)
    {
      ...
      AsyncQueueWorker(new LDAPPagedSearchProgress(callback, progress, obj->ld, DNbase,
                                                   scopeSearch, filterSearch,cookie_id,
                                                   pageSize, obj->cookies));
    }
