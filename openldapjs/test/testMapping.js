...
describe('String to JSON#searchTest', () => {
  ...
  it('should return the string as JSON', (next) => {
    JSONStruct.stringLDAPtoJSON(simpleLdif)
    .then((result) => {
  ...
  it('should return the string as JSON from a ldif with double attribute', (next) => {
    JSONStruct.stringLDAPtoJSON(doubleAttrLdif)
    .then((result) => {
      const JSONobjecttest = [{
        dn: 'ou=users,o=myhost,dc=demoApp,dc=com',
        attribute: [
          {type: 'objectClass', value: ['organizationalUnit', 'organizationalUnit']},
          {type: 'ou', value: ['users']},
        ],
      }];
      const shouldString = JSON.stringify(JSONobjecttest[0]);
   ...
   it('should return the string as JSON from a ldif with mixed up attribute', (next) => {
    JSONStruct.stringLDAPtoJSON(mixedAttrLdif)
    .then((result) => {
      const JSONobjecttest = [{
        dn: 'ou=users,o=myhost,dc=demoApp,dc=com',
        attribute: [
  ...
  it('should return the string as JSON from a ldif with 2 objects', (next) => {
    JSONStruct.stringLDAPtoJSON(moreObjLdif)
    .then((result) => {
      const JSONobjecttest = [{
        dn: 'ou=users,o=myhost,dc=demoApp,dc=com',
        attribute: [
  ...
  it('should return the string as JSON from a ldif with mixed up doubled attribute', (next) => {
    JSONStruct.stringLDAPtoJSON(mixedDoubledAttrLdif)
    .then((result) => {
      const JSONobjecttest = [{
        dn: 'ou=users,o=myhost,dc=demoApp,dc=com',
        attribute: [
          {type: 'objectClass', value: ['organizationalUnit1', 'organizationalUnit2']},
  ...
  it('should return an error if an attribute does not have a value', (next) => {
    JSONStruct.stringLDAPtoJSON(noValueLdif)
    .catch((err) => {
  ...
  it('should return an error if the ldif does not have any attributes', (next) => {
    JSONStruct.stringLDAPtoJSON(noAttrLdif)
    .catch((err) => {
      should.deepEqual(err.message, 'The string is not a LDAP structure');
      next();
    });
  ...
  it('should return an error if the dn is missing', (next) => {
    JSONStruct.stringLDAPtoJSON(noDnLdif)
    .catch((err) => {
      should.deepEqual(err.message, 'The string is not a LDAP structure');
      next();
    });
  ...
  it('should return an error if the dn has no value', (next) => {
    JSONStruct.stringLDAPtoJSON(emptyDnLdif)
    .catch((err) => {
      should.deepEqual(err.message, 'The string is not a LDAP structure');
      next();
    });
