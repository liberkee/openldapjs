  ...
  it('should return the string as JSON', () => {
    ...
    return adminLDAP.search(searchBase, scopeSearch.subtree, filterSpecificUser)
      .then((res) => {
        ...
        jsonRes.entries[0].attributes.forEach((element) => {
          jsonResults.push(element.value.value);
        });
      })
      .then(() => {
        should.deepEqual(jsonResults, stringResult);
      });
  });
    ...
    return Promise.all([search1, search2, search3])
      .then((results) => {
        results.forEach((element) => {
          ...
          const resultShoudBe = jsonRes.should.be.json;
        });
      });
  });
